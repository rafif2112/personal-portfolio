...
export default async function WakatimeStat({}: Props) {
  ...
  try {
    const weekStat = await getWakaTimeWeekStat();
    dailyAverage = weekStat.dailyAverage;
    totalSecond = weekStat.totalSecond;
  } catch (error) {
    console.error("Failed to fetch WakaTime week stats:", error);
  }

  try {
    const leaderBoard = await getWakaTimeLeaderBoard();
    globalLeaderBoard = leaderBoard.globalLeaderBoard;
    localLeaderBoard = leaderBoard.localLeaderBoard;
  } catch (error) {
    console.error("Failed to fetch WakaTime leaderboards:", error);
  }
